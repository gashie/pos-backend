...
const { OrderTkeOut } = require("../model/Order");
...
module.exports = {
  ...
  autoProcessOutletQuantity: async (req, oldvalue, newvalue, action, product_id, user_id) => {
    let payload = {}
    let progress = 0
    if (action === 'add') {
      payload.stock_quantity = Number(oldvalue) + Number(newvalue)
    } else {
      payload.stock_quantity = Number(oldvalue) - Number(newvalue)
    }
    const runupdate = await GlobalModel.Update(payload, 'outlet_inventory', 'product_id', product_id)
    if (runupdate.rowCount == 1) {
      CatchHistory({ payload: JSON.stringify(req.body), api_response: `User with id :${user_id} updated product quantity`, function_name: 'autoProcessQuantity', date_started: systemDate, sql_action: "UPDATE", event: "Update Prodcut Qty", actor: user_id }, req)
      return progress = 1
