...
exports.CreateSalaryBandAllowance = asynHandler(async (req, res, next) => {
    ...
    let results = await GlobalModel.Create(payload, 'group_band_allowance','');
    if (results.rowCount == 1) {
        CatchHistory({ payload: JSON.stringify(payload), api_response: `New salary band allowance added`, function_name: 'CreateSalaryBandAllowance', date_started: systemDate, sql_action: "INSERT", event: "Create salary band allowance", actor: userData.id }, req)
        return sendResponse(res, 1, 200, "Record saved", [])
    } else {
        CatchHistory({ payload: JSON.stringify(payload), api_response: `Sorry, error saving record: contact administrator`, function_name: 'CreateSalaryBandAllowance', date_started: systemDate, sql_action: "INSERT", event: "Create salary band allowance", actor: userData.id }, req)
...
exports.UpdateSalaryBandAllowance = asynHandler(async (req, res, next) => {
    let payload = req.body;
    let userData = req.user;
    ...
    if (runupdate.rowCount == 1) {
        ...
        CatchHistory({ payload: JSON.stringify(req.body), api_response: `Update failed, please try later-User with id :${userData.id} updated salary band allowance details`, function_name: 'UpdateSalaryAllowance', date_started: systemDate, sql_action: "UPDATE", event: "Update salary band allowance info", actor: userData.id }, req)
