...
exports.CreateSupplier = asynHandler(async (req, res, next) => {
    ...
    if (results.rowCount == 1) {
        ...
        CatchHistory({ payload: JSON.stringify(payload), api_response: `Sorry, error saving record: contact administrator`, function_name: 'CreateSupplier', date_started: systemDate, sql_action: "INSERT", event: "Create Supplier", actor: userData.id }, req)
...
exports.ViewTenantSupplier = asynHandler(async (req, res, next) => {
    ...
    CatchHistory({ api_response: `User with ${userData.id} viewed ${results.rows.length} supplier record's`, function_name: 'ViewTenantSupplier', date_started: systemDate, sql_action: "SELECT", event: "Supplier View", actor: userData.id }, req)

    sendResponse(res, 1, 200, "Record Found", results.rows)
})
exports.UpdateSupplier = asynHandler(async (req, res, next) => {
    let payload = req.body;
    let userData = req.user;
    ...
    if (runupdate.rowCount == 1) {
        ...
        CatchHistory({ payload: JSON.stringify(req.body), api_response: `Update failed, please try later-User with id :${userData.id} updated supplier details}`, function_name: 'UpdateSupplier', date_started: systemDate, sql_action: "UPDATE", event: "Update Category", actor: userData.id }, req)
